...
public class Polynom implements Polynom_able{
	...
	public Polynom (String s)  throws Exception{
		...
		for(int i=0;i<s.length();i++) {
			if(s.charAt(i)=='*'&&(s.charAt(i+1)=='x'||s.charAt(i+1)=='X')&&s.charAt(i+2)=='^') {
				coefficient=s.substring(Index_helper, i);    //retrieving the power
				if(isNumeric(coefficient)) {
					arr[Index_Arr++]= coefficient;                //we've got a string , now we'll check if it's a number
					Index_helper=i+3;			                  //state the new starting index to the char after '^'    
				}
				else {	

					throw new Exception("Error! This input is not following the form of: (Coefficient)*x^(Power)(+/-).....(+/-)(Coefficient)*x^(Power)"); //the coefficient is'nt a number	
				}
			}
				if((s.charAt(i)=='+'|| s.charAt(i)=='-')&&i!=0) { //retrieving the power
					if(s.charAt(i)=='+')
						power = s.substring(Index_helper,i); 
					else {                                         //checking if we need to include '-'
						power=s.substring(Index_helper,i);
					...
					if(isNumeric(power)) {
						...
						throw new Exception("Error! This input is not following the form of: (Coefficient)*x^(Power)(+/-).....(+/-)(Coefficient)*x^(Power)"); //the power is'nt a number
					}
				
			}
		}    //end for
		//checking the last chars between the last "*x^" to the end of the string
		power = s.substring(Index_helper,s.length());
		if(isNumeric(power))
			arr[Index_Arr++]=power;
		else
			throw new Exception("Error! This input is not following the form of: (Coefficient)*x^(Power)(+/-).....(+/-)(Coefficient)*x^(Power)"); //the power is'nt a number
		...
		int powerOfM;
		double coefficientOfM;
		for(int i =0 ;i<Index_Arr;i+=2) {
			coefficientOfM=Double.parseDouble(arr[i]);
			powerOfM=Integer.parseInt(arr[i+1]);
			Monom m =new Monom(coefficientOfM,powerOfM);
			add(m);
		}
	}
	*/
